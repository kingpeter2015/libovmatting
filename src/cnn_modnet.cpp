...
CNN_Modnet::CNN_Modnet(const CNNConfig& config) : _config(config), _bBgr(false)
{
    topoName = "CNN_Modnet";
...
bool CNN_Modnet::isBgrEnqueued()
{
    return _bBgr;
}

void CNN_Modnet::load()
{
...
void CNN_Modnet::reshape(cv::Size input_shape)
{
    _config.input_shape = input_shape;
    load();
}

void CNN_Modnet::submitRequest()
{
    if (!request)
    {
        return;
...
void CNN_Modnet::enqueue(const std::string& name, const cv::Mat& frame)
{
    if (!request)
    {
        return;
...
void CNN_Modnet::enqueueAll(const cv::Mat& frame, const cv::Mat& bgr)
{
    try
    {
        //1.init fields
    ...
    {
        std::cerr << "CNN_Modnet::enqueue():" << e.what() << '\n';
    }    
}

MattingObjects CNN_Modnet::fetchResults()
{
    ...
    try
    {
        ...
        //3.1. get alpha mat
        Blob::Ptr blobPha = request->GetBlob("1011");
