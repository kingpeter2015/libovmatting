...
void CNN_Background_V2::enqueue(const cv::Mat &frame, const cv::Mat &image)
{
    if (!request)
    {
...
CNN_Background_V2::CNN_Background_V2(const CnnConfig &config) : BaseMatting(config.is_async), _config(config)
{
...
MattingObjects CNN_Background_V2::fetchResults()
{
    ...
    if (blobFgr == nullptr)
    {
        THROW_IE_EXCEPTION << "CNN_Background_V2::fetchResults() Invalid fgr blob '";
    }
